...
import { convertImageToPdfLibs } from '../../server/helpers/convertPdf';
  ...
  AMPQ.consumeData(WORKER_NAME.CONVERT_MEDIA, async (msg, channel) => {
    try {
      ...
      switch (data.fileType) {
        case '.pdf':
           file = await convertPDFLibs(data);
          if (file) {
          ...
          break;
        case '.image':
          file = await convertImageToPdfLibs(data);
          if (data.type === 'userFile') {
            await UserFile.updateOne({
              _id: data._id,
            }, { $set: {
